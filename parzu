...
def postprocess(instream,outstream,options):
    ...
    if outputformat == "prolog":
        clean_parse = try_Popen(["perl", os.path.join(root_directory,"postprocessor","output_cleanup_sentnums.perl")], stdin=instream, stdout=cleanup_out, stderr=options['senderror'])
    
    else:
        clean_parse = try_Popen(["perl", os.path.join(root_directory,"postprocessor","output_cleanup.perl")], stdin=instream, stdout=cleanup_out, stderr=options['senderror'])
...
def generate_graphics(instream,outstream,options):
    ...
    conll2svg = try_Popen(["perl", '-I'+os.path.join(root_directory,'postprocessor','DepSVG'), os.path.join(root_directory,'postprocessor','DepSVG','conll_to_svg.perl'),'--dir='+os.getcwd()], cwd=os.path.join(root_directory,'postprocessor','DepSVG'), stdin=instream,stdout=outstream,stderr=options['senderror'])
